...
public class arenaManager : MonoBehaviour
{
        ...
        for (int y = 0; y < 9; ++y)
        {
            for (int x = 0; x < 9; ++x)
            {
                for (int a = 0; a < 9; ++a)
                {
                    ...
                    {
                        Debug.Log("gameManager.instance: Sudoku is incorrect (Horizontal) in line: " + (y + 1));
                        return false;
                    }
                }
            }
        ...
        for (int x = 0; x < 9; ++x)
        {
            for (int y = 0; y < 9; ++y)
            {
                for (int b = 0; b < 9; ++b)
                {
                    ...
                    {
                        Debug.Log("gameManager.instance: Sudoku is incorrect (Vertical) in line: " + (x + 1));
                        return false;
                    }
                }
            }
    ...
    private void loadArea()
    {
        ...
        for (int y = 0, i = 0, line = 1; y < 3;)
        {
            for (int x = 0; x < 3; ++x)
            {
                for (int w = 0; w < 3; ++w, ++i)
                {
                    ...
                    {
                        square[x, y].Init();
                        buffer[i] = square[x, y].getArea(w);
                    }
                    else if (line == 2 || line == 5 || line == 8)
                    {
                        square[x, y].Init();
                        buffer[i] = square[x, y].getArea(w + 3);
                    }
                    else if (line == 3 || line == 6 || line == 9)
                    {
                        square[x, y].Init();
                        buffer[i] = square[x, y].getArea(w + 6);
                    }
                }
        ...
        for (int y = 0, i = 0; y < 9; ++y) for (int x = 0; x < 9; ++x, ++i)
            {
