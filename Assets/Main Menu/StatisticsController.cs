...
public class StatisticsController : MonoBehaviour 
{
    ...
    {
        stats.moves += gameManager.instance.moves;
        stats.time += gameManager.instance.timer;
    ...
    {
        level.moves += gameManager.instance.moves;
        level.time += gameManager.instance.timer;
        if (isWon) level.wins++;
    }

    private void updateTxt()
    {
        stats.timeTxt.text = gameManager.instance.transformToTime(stats.time);
        stats.movesTxt.text = stats.moves.ToString();

        stats.easy.timeTxt.text = gameManager.instance.transformToTime(stats.easy.time);
        stats.easy.winsTxt.text = stats.easy.wins.ToString();
        stats.easy.movesTxt.text = stats.easy.moves.ToString();

        stats.medium.timeTxt.text = gameManager.instance.transformToTime(stats.medium.time);
        stats.medium.winsTxt.text = stats.medium.wins.ToString();
        stats.medium.movesTxt.text = stats.medium.moves.ToString();

        stats.hard.timeTxt.text = gameManager.instance.transformToTime(stats.hard.time);
